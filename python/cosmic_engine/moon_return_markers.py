"""Moon return marker sync for instanced sims.

First pass behavior:
- uses natal Moon sign hidden markers (`PlumAntics_CosmicEngineNatal_<Sign>MoonHidden`)
- applies one hidden return marker when current Moon sign == natal Moon sign
...
_NATAL_MOON_SUFFIX = "MoonHidden"
_RETURN_PREFIX = "PlumAntics_CosmicEngineReturns_"
_RETURN_MOON_SUFFIX = "MoonReturnHidden"
...
def _parse_natal_moon_sign_name(trait_name: str) -> Optional[int]:
    if not trait_name:
        return None
    text = str(trait_name)
    if not text.startswith(_NATAL_PREFIX):
        return None
    if "House" in text or not text.endswith(_NATAL_MOON_SUFFIX):
        return None
    sign_name = text[len(_NATAL_PREFIX) : -len(_NATAL_MOON_SUFFIX)]
    sign_index = _SIGN_TO_INDEX.get(sign_name)
    return int(sign_index) if sign_index is not None else None


def _parse_return_moon_marker_name(trait_name: str) -> Optional[int]:
    if not trait_name:
        return None
    text = str(trait_name)
    if not text.startswith(_RETURN_PREFIX):
        return None
    if "House" in text or not text.endswith(_RETURN_MOON_SUFFIX):
        return None
    sign_name = text[len(_RETURN_PREFIX) : -len(_RETURN_MOON_SUFFIX)]
    sign_index = _SIGN_TO_INDEX.get(sign_name)
    return int(sign_index) if sign_index is not None else None
...
def _rebuild_marker_cache() -> Dict[str, object]:
    ...
    for trait in _iter_manager_trait_tunings(_get_trait_instance_manager()):
        sign_index = _parse_return_moon_marker_name(_trait_name(trait))
...
def reset_moon_return_marker_cache() -> None:
...
def _equipped_natal_moon_sign_index(equipped_traits_with_ids: Iterable[tuple]) -> Optional[int]:
    found = None
    for equipped_trait, _equipped_tid in equipped_traits_with_ids:
        sign_index = _parse_natal_moon_sign_name(_trait_name(equipped_trait))
        if sign_index is None:
            continue
...
def sync_zone_moon_return_markers(
    *,
    ...
    if refresh_marker_cache:
        reset_moon_return_marker_cache()
    cache = _marker_cache()
    ...
    current_moon_sign = int(service.state.sign_index_by_body.get("Moon", 0)) % 12
    ...
    for sim_info in target_sim_infos:
        ...
        natal_moon_sign = _equipped_natal_moon_sign_index(equipped)
        if natal_moon_sign is None:
            continue
        summary["sims_with_natal_moon"] += 1

        desired_sign = int(current_moon_sign) if int(natal_moon_sign) == int(current_moon_sign) else None
        desired_tid = None
        desired_trait = None
            ...
            if active_sim_id is not None and sim_id is not None and int(active_sim_id) == int(sim_id):
                if removed_any:
                    ...
                    if _show_notification(
                        "Moon Return Ended",
                        "Your Moon Return window has closed.",
                        owner=sim_info,
                    ):
                        summary["return_notifications_shown"] += 1
                if added_any:
                    summary["return_events"] += 1
                    sign_name = SIGNS[int(current_moon_sign) % 12]
                    if _show_notification(
                        "Moon Return Started",
                        "Moon Return active: the Moon is in your natal Moon sign ({0}).".format(sign_name),
                        owner=sim_info,
                    ):
                        summary["return_notifications_shown"] += 1
